  ...
  <button type="submit" id="agregaDomicilio" data-action="addDomicilio"
    ...
    <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded ml-2">Buscar</button>
                                ...
                                <div id="tbl_domicilios" style="overflow-x: auto;" class="h-auto">
                                    ...
                                    <table id="oficinas-table" class="w-full text-sm text-left text-gray-500 mb-4">
    ...
    try {
        ...
        // Total de registros
        $queryTotal = $conexion->query("SELECT COUNT(*) FROM oficina");
        ...
        if ($busqueda !== '') {
            $query .= " WHERE nombre LIKE :busqueda OR direccion LIKE :busqueda";
        }
        $query .= " ORDER BY Id_Oficina ASC LIMIT :limit OFFSET :offset";
        ...
        if ($resultado) {
            foreach ($resultado as $fila) {
                ...
                echo '<td class="px-6 py-4">'.htmlspecialchars($fila['Id_Oficina']).'</td>';
                ...
                // Ver detalle
                echo '<button data-id="' . $fila['Id_Oficina'] . '" class="p-2 text-green-600 hover:text-green-800 rounded-full bg-green-100 hover:bg-green-200" title="Ver detalles">
                ...
                // Editar
                echo '<button data-id="' . $fila['Id_Oficina'] . '" class="p-2 text-blue-600 hover:text-blue-800 rounded-full bg-blue-100 hover:bg-blue-200" title="Editar">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                // Eliminar
                echo '<button data-id="' . $fila['Id_Oficina'] . '" class="delete-button p-2 text-red-600 hover:text-red-800 rounded-full bg-red-100 hover:bg-red-200" title="Eliminar">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
