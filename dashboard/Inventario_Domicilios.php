...
<head>
    ...
    <title>Domicilio | Inventario</title>
...
<div>
  ...
  <button type="submit" id="agregaDomicilio" data-action="addDomicilio"
    ...
    <input type="text" name="search" placeholder="Buscar Domicilio..." 
...
try {
    ...
    // Total de registros
    $queryTotal = $conexion->query("SELECT COUNT(*) FROM domicilios");
    ...
    if ($busqueda !== '') {
        $query .= " WHERE direccion LIKE :busqueda
                    OR municipio LIKE :busqueda
                    OR ubicacion LIKE :busqueda
                    OR escritorios LIKE :busqueda
                    OR sillas_de_escritorios LIKE :busqueda
                    OR sillas LIKE :busqueda
                    OR mesa_escritorio LIKE :busqueda
                    OR sillones LIKE :busqueda
                    OR mesa_de_centro LIKE :busqueda
                    OR cajoneras LIKE :busqueda
                    OR estantes LIKE :busqueda
                    OR otros LIKE :busqueda";
    }
    ...
    if ($resultado) {
        foreach ($resultado as $fila) {
           ...
           // echo '<td class="px-6 py-4">'.htmlspecialchars($fila['direccion']).'</td>';
            echo '<td class="px-6 py-4 text-blue-600 hover:underline cursor-pointer" onclick="verMobiliario('.$fila['id'].')">'.htmlspecialchars($fila['direccion']).'</td>';

            $empresas = [];

            $empresas = [];

            if (!empty($fila['empresa1'])) $empresas[] = htmlspecialchars($fila['empresa1']);
            if (!empty($fila['empresa2'])) $empresas[] = htmlspecialchars($fila['empresa2']);
            ...
            // Elegimos 2 colores distintos al azar para la fila
            shuffle($colores);
            
            echo '<td class="px-6 py-4">';
            foreach ($empresas as $index => $empresa) {
                $color = $colores[$index % count($colores)];
                echo '<span class="inline-block '.$color.' text-xs font-semibold mr-2 px-2.5 py-0.5 rounded">'
                    . $empresa .
                    '</span>';
            }
            echo '</td>';
            ...
            echo '<td class="px-6 py-4">'.htmlspecialchars($fila['escritorios']).'</td>';
            echo '<td class="px-6 py-4">'.htmlspecialchars($fila['sillas_de_escritorios']).'</td>';
            echo '<td class="px-6 py-4">'.htmlspecialchars($fila['sillas']).'</td>';
            echo '<td class="px-6 py-4">'.htmlspecialchars($fila['mesa_escritorio']).'</td>';
            echo '<td class="px-6 py-4">'.htmlspecialchars($fila['sillones']).'</td>';
            echo '<td class="px-6 py-4">'.htmlspecialchars($fila['mesa_de_centro']).'</td>';
            echo '<td class="px-6 py-4">'.htmlspecialchars($fila['cajoneras']).'</td>';
            echo '<td class="px-6 py-4">'.htmlspecialchars($fila['estantes']).'</td>';


            echo '<td class="px-6 py-4 text-center flex justify-center space-x-2">';
            
            // Botón Ver detalle (ícono ojo)
            echo '<button data-drawer-target="drawer-right-example" data-drawer-show="drawer-right-example" data-drawer-placement="right" aria-controls="drawer-right-example" data-row-id="' . $fila['id'] . '" type="button" class="p-2 text-green-600 hover:text-green-800 rounded-full bg-green-100 hover:bg-green-200 focus:outline-none focus:ring-2 focus:ring-green-500" title="Ver detalles">
            ...
            echo '<button data-id-domicilio="' . $fila['id'] . '" data-modal-target="crud-edit" data-modal-toggle="crud-edit" type="button" class="p-2 text-blue-600 hover:text-blue-800 rounded-full bg-blue-100 hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-500" title="Editar">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536M9 11l6-6 3 3-6 6H9v-3z"/>
            </svg>
            </button>';
            
            // Botón Eliminar (ícono cruz)
            echo '<button data-id-domicilio= "'.$fila['id'].'" type="button" class="delete-button p-2 text-red-600 hover:text-red-800 rounded-full bg-red-100 hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-red-500" title="Eliminar">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
<!-- Modal para ver mobiliario relacionado -->
<div id="modalMobiliario" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
    <div class="bg-white rounded-lg shadow-lg w-11/12 md:w-3/4 max-h-[80vh] overflow-y-auto p-6">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-bold text-gray-800">Mobiliario Relacionado</h2>
            <button onclick="cerrarModalMobiliario()" class="text-gray-600 hover:text-gray-800 text-lg font-bold">&times;</button>
        </div>
        <div id="contenidoMobiliario" class="overflow-x-auto">
...
<script>
function verMobiliario(idDomicilio) {
    // Mostrar modal
    document.getElementById('modalMobiliario').classList.remove('hidden');

    // Mostrar mensaje de carga
    const contenido = document.getElementById('contenidoMobiliario');
    contenido.innerHTML = '<p class="text-gray-600 text-sm">Cargando mobiliario...</p>';

    // Llamar a PHP
    fetch('domicilio/get_mobiliario.php?id_domicilio=' + idDomicilio)
        .then(response => response.text())
        .then(data => {
            contenido.innerHTML = data || '<p class="text-gray-600 text-sm">No hay mobiliario registrado en este domicilio.</p>';
        })
        .catch(error => {
            contenido.innerHTML = '<p class="text-red-600">Error al cargar el mobiliario.</p>';
            console.error(error);
        });
}

function cerrarModalMobiliario() {
    document.getElementById('modalMobiliario').classList.add('hidden');
}
</script>
